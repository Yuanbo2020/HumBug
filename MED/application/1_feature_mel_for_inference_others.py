...
def run_jobs():
    ...
    source_audio_space = r'E:\Yuanbo\Code\26_MED_MSC\0_Dataset'

    name = 'audio'
    name = '1_audio_aedes_albopictus'
    name = '2_Audio_Stella_Nucr13_Aug18_midframe'

    source_audio_dir = os.path.join(source_audio_space, name)
    ...
    for (file_n, audio_file_name) in enumerate(os.listdir(source_audio_dir)):
        ...
        if not os.path.exists(output_feature_path):
            ...
            if length >= min_len:
                ...
                if audiodata.ndim < 2:  # 1_audio_aedes_albopictus 本来就是双声道，而NIPS2021 audio 是单

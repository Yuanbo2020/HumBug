...
def get_train_test_multispecies(df_all, classes, random_seed, train_fraction=0.75):
    ...
    if not os.path.isfile(os.path.join(config.dir_out_MSC, pickle_name_train)):
        ...
        for species in classes:
            ...
            species_recordings[species] = len(pd.unique(
                df_all[df_all.species == species].name))  # Number of unique audio recordings (and hence mosquitoes)
...
def get_test_from_df(df_test_A, df_test_B, debug=False, pickle_name=None):
    ...
    if not os.path.isfile(os.path.join(config.FeatB_space, pickle_name_test)):
        print('Extracting test features...')
        ...
        with open(os.path.join(config.FeatB_space, pickle_name_test), 'wb') as f:
            pickle.dump(log_mel_feat_test, f)
            print('Saved features to:', os.path.join(config.FeatB_space, pickle_name_test))
    else:
        print('Loading test features found at:', os.path.join(config.FeatB_space, pickle_name_test))
        with open(os.path.join(config.FeatB_space, pickle_name_test), 'rb') as input_file:
            log_mel_feat = pickle.load(input_file)
